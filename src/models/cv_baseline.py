...
for fold, (train_idx, valid_idx) in enumerate(skf.split(X, y), 1):
    ...
    model = lgb.train(
        ...
        callbacks=[
            ...
            lgb.log_evaluation(period=0)  # disables per-iteration logs
        ]
    )
    ...
    logger.info(
        f"Fold {fold} - Log Loss: {fold_logloss:.4f}, AUC: {fold_auc:.4f}"
    )
...
logger.info(f"Mean Log Loss: {np.mean(log_losses):.4f}")
logger.info(f"Std Log Loss:  {np.std(log_losses):.4f}")
logger.info(f"Mean AUC:      {np.mean(auc_scores):.4f}")
logger.info(f"Std AUC:       {np.std(auc_scores):.4f}")
